...
- **Web pages** — use the local firecrawl script (no API key, no cost):
  ```bash
  # Fetch a single page → markdown
  python3 ~/.openclaw/workspace/scripts/firecrawl.py scrape <url>

  # Fetch with metadata (title, author, description) as JSON
  python3 ~/.openclaw/workspace/scripts/firecrawl.py scrape <url> --format json --out raw.json

  # Multiple URLs in parallel
  python3 ~/.openclaw/workspace/scripts/firecrawl.py batch <url1> <url2> --out results.json

  # Crawl a full site (e.g. all press releases on a company's newsroom)
  python3 ~/.openclaw/workspace/scripts/firecrawl.py crawl <url> --depth 2 --limit 50 --out crawl.json
  ```
  Falls back to Steel Browser (localhost:3000) automatically for JS-heavy pages.
