...
- **For fetching specific pages** (company websites, bio pages, news articles, press releases),
  use the local firecrawl script — no API key, no cost:
  ```bash
  python3 ~/.openclaw/workspace/scripts/firecrawl.py scrape <url> --format json
  ```
  Auto-falls back to Steel Browser (localhost:3000) for JS-heavy pages.
